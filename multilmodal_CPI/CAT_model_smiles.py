...
class CAT_smiles(nn.Module):
    def __init__(self,
        ...
        super(CAT_smiles, self).__init__()

        norm_layer = partial(nn.LayerNorm, eps=1e-6)
        self.pos_embed_img1 = nn.Parameter(torch.zeros(1, 256, 256))
        self.pos_embed_pro = nn.Parameter(torch.zeros(1, 256, 256))
        self.pos_embed_smiles = nn.Parameter(torch.zeros(1, 256, 256))
        self.pos_drop_img1 = nn.Dropout(p=drop_ratio)
        ...
        dpr_img1 = [x.item() for x in torch.linspace(0, drop_ratio, depth)]
        self.encoder_img1 = nn.Sequential(*[
            Block(dim=embed_dim,
                  mlp_ratio=mlp_ratio,
                  drop_ratio=dpr_img1[i],
                  )
            for i in range(depth)
        ])
        self.norm_img1 = norm_layer(embed_dim)
        depth_pro = 1
        ...
        self.img_backbone1 = nn.Sequential(  # 3*256*256
    ...
    def forward_img_features(self, x):
        B, _, _, _ = x.shape
        # x = self.cnn(x)
        x = self.img_backbone1(x)
        x = x.reshape(B, 256, -1)
        x = self.pos_drop_img1(x + self.pos_embed_img1)
        x = self.encoder_img1(x)
        x = self.norm_img1(x)
        return x
